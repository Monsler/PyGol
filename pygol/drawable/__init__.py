...
class ProgressBar(Drawable):
    ...
    def __init__(self, x: int, y: int, width: int, height: int, base_progress: int):
        super().__init__()
        self.x = x
        self.y = y
        self.width = width
        ...
        self.rect_pg = pygame.Rect(x+5, y+5, base_progress / width-5, height-5)
        
    def on_draw(self, canvas):
        if self.visible:
            self.rect_bg = pygame.Rect(self.x, self.y, self.width, self.height)
            pygame.draw.rect(canvas, self.bg, self.rect_bg)
            self.rect_pg = pygame.Rect(self.x+5, self.y+5, self.width / self.base_progress-5, self.height-5)
            pygame.draw.rect(canvas, self.fg, self.rect_pg)
    def set_progress(self, a: int):
        self.base_progress = a
    
    def extend_progress_by(self, a: int):
        self.base_progress += a
