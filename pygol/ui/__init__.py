...
class Window:
    def __init__(self, title: str='PyGol Window', width: int=500, height: int=400, is_fullscreen: bool=False, is_resizable: bool=True) -> None:
    ...
    def _internal_main(self):
        ...
        self.pygame_window = pygame.display.set_mode((self.width, self.height))
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                   ...
                   threading.Thread(target=check_clicks, args=(event.pos, 'ended', self, event)).start()
                    ...
                    if self.tf_evt is not None: self.tf_evt(event.key)
