...
def perform_with_delay(millis, target, times=1) -> multiprocessing.Process:
    proc = multiprocessing.Process(target=_run_async_void, args=(millis, target, times), daemon=True)
    proc.start()
    return proc
